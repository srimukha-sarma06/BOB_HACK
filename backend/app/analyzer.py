...
RISK_KEYWORDS = ["auth", "security", "payment", "billing", "db", "database", "migration", "crypto", "secret", "token", "permission"]
...
def find_entrypoints(repo_path: Path, files: List[Path]) -> List[str]:
    candidates = []
    names = {"main.py", "app.py", "server.py", "index.js", "server.js", "main.ts", "index.ts", "App.jsx", "App.tsx"}
    for file in files:
...
def score_file(repo_path: Path, file: Path, test_files: List[str]) -> Tuple[int, str, bool]:
    ...
    if file.suffix.lower() in CODE_EXTS:
        import_count = sum(1 for line in lines if line.strip().startswith(("import ", "from ", "require(", "const ")))
        if import_count > 18:
            score -= 12
...
def group_files(repo_path: Path, files: List[Path]) -> Dict[str, List[str]]:
    groups = {"entrypoints": [], "routes": [], "tests": [], "docs": [], "config": []}
    for file in files:
        r = rel(repo_path, file)
        lower = r.lower()
        if file.name.lower() in {"main.py", "app.py", "server.py", "index.js", "main.ts", "app.jsx", "app.tsx"}:
        ...
        if file.name.lower() in {"requirements.txt", "pyproject.toml", "package.json", "dockerfile", ".env.example", "pytest.ini", "tsconfig.json", "vite.config.js"}:
...
def analyze_repo(repo_path: Path, repo_id: str, repo_name: str, repo_url: str) -> Dict[str, Any]:
    ...
    test_files = [r for r in rel_files if "test" in r.lower() or "spec" in r.lower()]
    docs = [r for r in rel_files if r.lower().endswith((".md", ".rst")) or r.lower().startswith("docs/")]
    ...
    for file in files:
        ...
        item = {"file": rel(repo_path, file), "score": score, "reason": reason}
        if is_risky:
            risky.append({"file": item["file"], "reason": reason})
        else:
            scored.append(item)

    good_first_files = sorted(scored, key=lambda x: x["score"], reverse=True)[:8]
    risky_files = sorted(risky, key=lambda x: x.get("file", ""))[:8]
    ...
    if not test_files:
        onboarding_score -= 12
    onboarding_score = max(0, min(100, onboarding_score))

    estimated_manual = 90 + min(len(files), 500) // 3 + (0 if readme_exists else 45) + (0 if test_files else 45)
    estimated_firstpr = max(18, estimated_manual // 8)
