...
INDEXABLE_EXTS = {
    ...
    ".go", ".rs", ".rb", ".php", ".cs", ".md", ".txt", ".yaml", ".yml", ".json"
}
...
def collect_chunks(repo_path: Path, max_chunks: int = 1200) -> List[Dict[str, Any]]:
    ...
    for file in iter_files(repo_path):
        if file.suffix.lower() not in INDEXABLE_EXTS:
            continue
...
class LocalVectorStore:
    ...
    def build(self, repo_id: str, repo_path: Path) -> Dict[str, Any]:
        ...
        return {"backend": self.backend_name, "chunks_indexed": len(records)}
...
class ChromaVectorStore:
    ...
    def build(self, repo_id: str, repo_path: Path) -> Dict[str, Any]:
        ...
        # Clear existing collection contents best-effort
        try:
        ...
        chunks = collect_chunks(repo_path)
        if not chunks:
            return {"backend": self.backend_name, "chunks_indexed": 0}
        ...
        return {"backend": self.backend_name, "chunks_indexed": len(ids)}

    def query(self, repo_id: str, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        collection = self._collection(repo_id)
        result = collection.query(query_embeddings=[embed_text(query)], n_results=top_k)
